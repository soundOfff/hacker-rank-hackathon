...
- **claim_status**:
  ...
  - `not_enough_information` — you genuinely cannot tell what the claimed part looks like: it is not shown, too blurry/dark/occluded, contents hidden, **or** a multi-image set is internally inconsistent (different objects/vehicles) so the subject cannot be identified. Conflicting or untrustworthy evidence is **not** by itself a reason for `not_enough_information`; if a clear, valid view of the claimed part settles the claim, prefer `supported`/`contradicted`. Rule of thumb: **a clear photo of the wrong object → `contradicted`; an occluded part or an identity-ambiguous multi-image set → `not_enough_information`.**
...
Multi-image rule: consider each image separately. At least one relevant image must show the claimed object/part clearly enough to meet the evidence standard. However, when the claim depends on the subject's identity and the images are mutually inconsistent — they depict different objects/vehicles, or the damaged photo cannot be reconciled with a full-view photo — treat the set as **not** meeting the evidence standard (`evidence_standard_met=false`, `not_enough_information`), even if one image is individually clear. This applies only to genuine object/identity conflicts, not to harmless angle/scene/lighting differences or an unrelated context shot. For multi-part claims, judge the claimed parts together and report the most relevant issue/part.
